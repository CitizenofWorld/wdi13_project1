...
// functions used in getCol(); to determin different win scenarios
...
var p1RowWin = function() {
  ...
  if (p1Row0Total === 3) {
    p1Win(); // adds ninja dance gif and winner css to the win div
    changeP1Score();
...
var p2RowWin = function() {
  ...
  //determins which has 3 in a row for across winner and calls a function to display who won
  if (p2Row0Total === 3) {
    p2Win(); // adds cat ninja gif and winner css to the win div
    changeP2Score();
...
var p1ColWin = function() {
  ...
  if (p1Col0Total === 3) {
    p1Win(); //adds lego ninja gif to win div
    changeP1Score();
...
var p2ColWin = function() {
  ...
  //determins which has 3 in a row for column winner and calls a function to display who won
  if (p2Col0Total === 3) {
    p2Win(); //adds hand ninja gif to win div
    changeP2Score();
...
var p1BoardSquares = document.querySelectorAll(".col"); //find each p1 clicked square for reset
var p2BoardSquares = document.querySelectorAll(".col"); //find each p12 clicked square for reset
...
var roundNum = document.querySelector(".round-num"); // location of the round count div

// if player 1 wins function adds player-1-win class to be displayed
var p1Win = function() {
  winnerDiv.style.zIndex = "0";
  winnerDiv.classList.toggle("player-1-win");
  gameBoard.style.zIndex = "-1";
  restartDiv.style.zIndex = "0";
};
// if player 2 wins function adds player-2-win class to be displayed
var p2Win = function() {
  winnerDiv.style.zIndex = "0";
  winnerDiv.classList.toggle("player-2-win");
  gameBoard.style.zIndex = "-1";
  restartDiv.style.zIndex = "0";
};
// if no player wins function adds draw class to be displayed
var tie = function() {
  winnerDiv.style.zIndex = "0";
  winnerDiv.classList.toggle("draw");
  gameBoard.style.zIndex = "-1";
  restartDiv.style.zIndex = "0";
};
...
//changes players score on the score board
...
var resetPlayerBoard = function() {
  newWinnerDiv.addEventListener("click", function() {
    winnerDiv.classList.remove("player-1-win");
    winnerDiv.classList.remove("player-2-win");
    winnerDiv.classList.remove("draw");
